    ...
    async function loadMorePosts(numberOfPostsAlready) {
        ...
        if (isset($_GET["jeu"])){
            ...
            if($_GET["jeu"] != ""){?>
                ...
                var AJAXresult = await fetch("./BoutPage/MoreRecommandation.php?firstPost="+numberOfPostsAlready+"&jeu=<?php echo $_GET["jeu"] ;?>");
            <?php
            } else {?>
               var AJAXresult = await fetch("./BoutPage/MoreRecommandation.php?firstPost=" + numberOfPostsAlready);
            <?php
            }
